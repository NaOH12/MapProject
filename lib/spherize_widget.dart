...
//    print("    number of visited faces is $facecount");
  ...
  void paint(Canvas canvas, Size size) {
    ...
    int topOffset = (size.height - scaledSize.height) ~/ 2;
    for (Tile tile in tiles) {
      if (tile.loadError == false &&
        ...
        int scaledWidth = (tileSize.width * tile.level.scale).ceil() + 1;
        int scaledHeight = (tileSize.height * tile.level.scale).ceil() + 1;
    ...
    final double adjustLeftOffset = -outOfBoundsX * (1 - spherizeEffect);
    final double adjustTopOffset =
        ((topOffset.toDouble() + outOfBoundsY) * spherizeEffect) - outOfBoundsY;
