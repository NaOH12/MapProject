...
abstract class TileMarkerProvider {
  ...
  Future<List<List<Marker>>> getMarkerData(List<Coords<num>> coords,
...
class TilePostProvider extends TileMarkerProvider {
  TilePostProvider();
  ...
  @override
  Future<List<List<PostMarker>>> getMarkerData(List<Coords<num>> coords,
      ...
      for (final post in tilePosts) {
        ...
        list[i].add(PostMarker(
            ...
            builder: (options.postBuilder != null)
                ? options.postBuilder
                : defaultBuilder(post, 30, 20)));
      }
      tiles[i].markers = list[i];
      i++;
    }
    return list;
  }
